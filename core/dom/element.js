...
CKEDITOR.tools.extend( CKEDITOR.dom.element.prototype, {
	 ...
	 * @returns {Boolean}
	 */
	contains: ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.webkit ?
		function( node ) {
			var $ = this.$;
	...
	getHtml: function() {
		...
		return ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? retval.replace( /<\?[^>]*>/g, '' ) : retval;
	},
	 ...
	 */
	getComputedStyle: ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ?
	 ...
	 */
	getTabIndex: ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ?
		function() {
	...
	getName: function() {
		...
		if ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 && !( document.documentMode > 8 ) ) {
	...
	copyAttributes: function( dest, skipAttributes ) {
		...
		for ( var n = 0; n < attributes.length; n++ ) {
			...
			else if ( attribute.specified || ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 && attribute.nodeValue && attrName == 'value' ) ) {
	...
	CKEDITOR.dom.element.prototype.setSize = function( type, size, isBorderBox ) {
		if ( typeof size == 'number' ) {
			if ( isBorderBox && !( CKEDITOR.env.ie && CKEDITOR.env.version < 11 && CKEDITOR.env.quirks ) )
