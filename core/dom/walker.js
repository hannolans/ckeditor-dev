...
(function() {
	...
	CKEDITOR.dom.walker.bogus = function( isReject ) {
		...
		return function( node ) {
			var isBogus = !( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? node.is && node.is( 'br' ) : node.getText && tailNbspRegex.test( node.getText() );
	...
	CKEDITOR.dom.element.prototype.getBogus = function() {
		...
		if ( tail && ( !( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? tail.is && tail.is( 'br' ) : tail.getText && tailNbspRegex.test( tail.getText() ) ) ) {
			return tail;
		}
		return false;
	};
