...
(function() {
	...
	CKEDITOR.dom.walker.bogus = function( isReject ) {
		...
		return function( node ) {
			var isBogus = !( CKEDITOR.env.ie && CKEDITOR.env.version <= 10 ) ? node.is && node.is( 'br' ) : node.getText && tailNbspRegex.test( node.getText() );
	...
	CKEDITOR.dom.element.prototype.getBogus = function() {
		...
		if ( tail && ( !( CKEDITOR.env.ie && CKEDITOR.env.version <= 10 ) ? tail.is && tail.is( 'br' ) : tail.getText && tailNbspRegex.test( tail.getText() ) ) ) {
			return tail;
		}
		return false;
	};
