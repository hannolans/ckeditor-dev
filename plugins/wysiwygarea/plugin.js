	...
	CKEDITOR.plugins.add( 'wysiwygarea', {
		init: function( editor ) {
			...
			editor.addMode( 'wysiwyg', function( callback ) {
				...
				src = CKEDITOR.env.air ? 'javascript:void(0)' : ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? 'javascript:void(function(){' + encodeURIComponent( src ) + '}())'
					:
					'';
				...
				var useOnloadEvent = ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.gecko;
				if ( useOnloadEvent )
	...
	function onDomReady( win ) {
		...
		if ( editor.config.disableObjectResizing ) {
			try {
				...
				this.attachListener( this, ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? 'resizestart' : 'resize', function( evt ) {
					evt.data.preventDefault();
				});
			}
		}

		if ( CKEDITOR.env.gecko || CKEDITOR.env.ie && CKEDITOR.env.version < 11 && editor.document.$.compatMode == 'CSS1Compat' ) {
		...
		proto: {
			setData: function( data, isSnapshot ) {
				...
				if ( isSnapshot ) {
					...
					var bootstrapCode =
						'<script id="cke_actscrpt" type="text/javascript"' + ( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? ' defer="defer" ' : '' ) + '>' +
							...
							'function onload(){' +
								...
								'wasLoaded=1;' +
							'}' +
							( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? 'onload();' : 'document.addEventListener("DOMContentLoaded", onload, false );' ) +
						'</script>';
