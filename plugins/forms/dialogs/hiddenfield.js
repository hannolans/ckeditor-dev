	...
	return {
		...
		onOk: function() {
			var name = this.getValueOf( 'info', '_cke_saved_name' ),
				...
				element = CKEDITOR.env.ie && CKEDITOR.env.version < 11 && !( CKEDITOR.document.$.documentMode >= 8 ) ? editor.document.createElement( '<input name="' + CKEDITOR.tools.htmlEncode( name ) + '">' ) : editor.document.createElement( 'input' );
