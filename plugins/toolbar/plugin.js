		...
		toolbarFocus: {
			...
			exec: function( editor ) {
				if ( editor.toolbox ) {
					...
					if ( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.air )
						setTimeout( function() {
		...
		init: function( editor ) {
			...
			editor.on( 'uiSpace', function( event ) {
				...
				if ( editor.config.toolbarCanCollapse && editor.elementMode != CKEDITOR.ELEMENT_MODE_INLINE ) {
					...
					editor.setKeystroke( CKEDITOR.ALT + ( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.webkit ? 189 : 109 ) /*-*/, 'toolbarCollapse' );
