	...
	function initClipboard( editor ) {
		...
		function addListenersToEditable() {
			...
			editable.on( mainPasteEvent, function( evt ) {
				if ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 && preventBeforePasteEvent )
					return;
			...
			CKEDITOR.env.ie && CKEDITOR.env.version < 11 && editable.on( 'paste', function( evt ) {
			...
			editable.attachListener( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? editable : editor.document.getDocumentElement(), 'mouseup', function() {
		...
		function getClipboardDataByPastebin( evt, callback ) {
			...
			var pastebin = new CKEDITOR.dom.element(
				( CKEDITOR.env.webkit || editable.is( 'body' ) ) && !( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.opera ) ? 'body' : 'div', doc );
			...
			if ( ff3x ) {
				...
				if ( CKEDITOR.env.webkit ) {
					...
					editable.getAscendant( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) || CKEDITOR.env.opera ? 'body' : 'html', 1 ).append( pastebin );
				}
		...
		function onKey( event ) {
			...
			switch ( event.data.keyCode ) {
				...
				case CKEDITOR.SHIFT + 45: // SHIFT+INS
					...
					!( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) && editable.fire( 'beforepaste' );
		...
		function stateFromNamedCommand( command ) {
			...
			if ( command == 'Paste' ) {
				...
				CKEDITOR.env.ie && CKEDITOR.env.version < 11 && ( preventBeforePasteEvent = 1 );
				try {
