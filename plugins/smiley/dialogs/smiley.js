...
CKEDITOR.dialog.add( 'smiley', function( editor ) {
	...
	var html = [
		...
		CKEDITOR.env.ie && CKEDITOR.env.version < 11 && CKEDITOR.env.quirks ? ' style="position:absolute;"' : '',
		'><tbody>'
		];
	...
	for ( i = 0; i < size; i++ ) {
		...
		( ( CKEDITOR.env.ie && CKEDITOR.env.version < 11 ) ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ), '>' +
